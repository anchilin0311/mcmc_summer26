...
# Generate observed blurred data [y = Ax + eps] A is the NxN gaussian blurring matrix
...
def gibbs(delta_initial, lam_initial, observed_data, A, L, N, alpha_del, beta_del, alpha_lam, beta_lam, n_iter=10):
    ...
    # sampling the image state x given (delta, lambda, y)
    for idx in range(n_iter):
        K = np.linalg.cholesky(lam_samples[idx]*np.dot(np.transpose(A),A) + delta_samples[idx]*L) # KK^T = Q_post

        # Ky = lambda A^T y_observe + w --> K^T x = y and then solve for y
        ...
        # sampling hyperparameters (lamdba, delta) given x
        lam_shape = N/2 + alpha_lam
        delta_shape = N/2 + alpha_del
        lam_rate = np.sum(0.5 * (observed_data - np.dot(A,theta_samples[:,idx]))**2) + beta_lam # ||y - Ax||^2
        delta_rate = 0.5 * np.dot(theta_samples[:,idx],np.dot(L,theta_samples[:,idx])) + beta_del # x^T L x
...
# Explicitly compute -log pi(theta | data)
